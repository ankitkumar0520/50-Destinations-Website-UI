  ...
  implements OnInit, AfterViewInit, OnDestroy
{
  ...
  getAllDistricts() {
    this.apiService.get('Master/GetAllDistricts').subscribe((res: any) => {
      if (res) {
        this.districts = res.map((district: any) => ({
          id: district.districtid,
          name: district.districtname,
          value: district.districtcode.toLowerCase(),
          description: district.description,
          image:
            district.media.find((media: any) => media.iscover)?.mediaurl || '',
          keyPlaces: district.tagline ? district.tagline.split(',') : [],
          population: `${district.population}+`,
          elevation: `${district.elevation} ft`,
        }));
